...
int GPS_create_bin(char* param, int run_times, bool updateCh)
{
	// debug 
	// print param
	fprintf(stderr, "Param: %s\n", param);
	...
	sprintf(filepath, "%s%s%s%d%s", cwd,"\\binfile\\", outfile,run_times, ".txt");
	...
	grx = incGpsTime(grx, 0.1); // 1st
	if (run_times > 0) {
		for (int t = 1; t < run_times * 10; t++)
			grx = incGpsTime(grx, 0.1); // 1st
	}
	// for loop caculate iq_buff and write to stream file
	//for (iumd=1; iumd<numd; iumd++)
	for (iumd = 1; iumd < 11; iumd++)
	{
		for (i=0; i<MAX_CHAN; i++)
		{
			if (chan[i].prn>0)
		...
		for (isamp=0; isamp<iq_buff_size; isamp++)
		{
			...
			//fprintf(stderr, "iq_buff: %d, I->%d, Q->%d\n", sizeof(iq_buff)/sizeof(iq_buff[0]), i_acc, q_acc);
		}
		...
		//	sprintf(filepath,"%s%s%s%d%s", cwd, "\\binfile\\", outfile, filenum, ".bin");
		//	fprintf(stderr, "\nFile no = %d, %s\n", filenum, filepath);
		//	// close previous file
		//	fclose(fp);
...
//{
//
//	char* param = "-e brdc0010.22n -u circle.csv -o filename -t 2022/04/11,10:10:0.0";
//	printf("%s\n", param);
//	int res = GPS_create_bin(param);
